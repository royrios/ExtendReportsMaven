...
public class pcOne {
	...
	static Logger log = LoggerFactory.getLogger(pcOne.class);
	...
	public void startReportTwo() throws MalformedURLException{
		...
		extent = new ExtentReports("C:\\wselenium\\reporting\\HDIReport.html", false);
		...
		try{
			ClassLoader cLoader = pcOne.class.getClassLoader();
		...
		String ruta= prop.getProperty("config.url");

		numeroSeriePcOne=prop.getProperty("config.txtNumeroSerie");
	...
	public void CotizadorTestTwo()  {
		...
		logger = extent.startTest("Cotizador Autos HDI PC ONE");
		//generarReporteExterno();
