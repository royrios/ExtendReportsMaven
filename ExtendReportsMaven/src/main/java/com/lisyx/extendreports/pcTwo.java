...
public class pcTwo {
	...
	static Logger log = LoggerFactory.getLogger(pcTwo.class);
	@BeforeTest
	public void startReport() throws MalformedURLException{
		...
		//extent = new ExtentReports ("/Users/leonardo/Documents/Selenium/STMExtentReport.html", false);
		...
		*/
		extent = new ExtentReports ("C:\\wselenium\\reporting\\HDIReportTwo.html", false);
		//extent.addSystemInfo("Environment","Environment Name")
		extent
		.addSystemInfo("Host Name", "SoftwareTestingMaterial")
		.addSystemInfo("Environment", "Automation Testing")
		...
		//extent.loadConfig(new File("/Users/leonardo/Documents/Selenium/ExtentReportExample/extent-config.xml"));
		extent.loadConfig(new File("C:\\Nuevo\\ExtendReportsMaven\\extent-config.xml"));

		//baseUrl = "https://autos.staging.hdi.com.mx/";
		baseUrl = "https://autos.staging.hdi.com.mx/";
		try{
			ClassLoader cLoader = pcTwo.class.getClassLoader();
		...
		//System.setProperty("webdriver.gecko.driver", driverPath+"geckodriver");
				...
				String browserPcTwo=prop.getProperty("config.browsert");
				String driverRoutePcTwo = prop.getProperty("config.driverRoutet");
			
				switch (browser) {

				case  "firefox":
					capabilityPcTwo= DesiredCapabilities.firefox();
					System.setProperty("webdriver.gecko.driver", driverRoutePcTwo);

					break;

				case  "chrome":
					System.setProperty("webdriver.chrome.driver", driverRoutePcTwo);
					capabilityPcTwo= DesiredCapabilities.chrome();

					break;
				case  "internet explorer":
					capabilityPcTwo= DesiredCapabilities.internetExplorer();
				...
				capabilityPcTwo.setBrowserName(browser);
				capabilityPcTwo.setPlatform(Platform.WIN8);
				

				String ruta= prop.getProperty("config.urlt");
				String rutaPcTwo= prop.getProperty("config.urlt");

				 numeroSeriePcOne=prop.getProperty("config.txtNumeroSerieT");
				 rfcOne=prop.getProperty("config.rfc");

				//	driver = nesw RemoteWebDriver(new URL("http://192.168.0.107:5556/wd/hub"), capability);
				driver = new RemoteWebDriver(new URL(ruta), capabilityPcTwo);
				//driverPcTwo=  new RemoteWebDriver(new URL("http://192.168.0.51:5556/wd/hub"), capabilityPcTwo);
			//s	driverPcTwo.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
				
				//logger = extent.startTest("Cotizador Autos HDI");
	...
	public void CotizadorTestTwo()  {
		...
		//generarReporteExterno();
		logger = extent.startTest("Cotizador Autos HDI PC TWO");
		//generarReporteExterno();
